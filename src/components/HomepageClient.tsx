...
function StatCounter({ value, label, shouldStart }: { value: string; label: string; shouldStart: boolean }) {
  ...
  return (
    <div className="border-t border-black/10 pt-6 md:pt-7">
      <p className="text-[88px] font-light leading-[0.96] md:text-[116px]">{displayValue}{parsed.suffix}</p>
      <p className="mt-3 text-[30px] font-light leading-[1.17] text-black/80 md:text-[22px]">{label}</p>
    </div>
  )
}
...
function StarBurst() {
  return (
    <svg width="283" height="364" viewBox="0 0 283 364" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-auto w-[130px] md:w-[170px] lg:w-[220px]">
      <path d="M182.717 0L192.158 132.995L249.005 12.3913L209.765 139.816L306.34 47.8919L223.719 152.537L346.98 101.707L232.136 169.439L365.434 166.569L233.878 188.241L359.212 233.717L228.711 206.402L329.153 294.083L217.332 221.47L279.318 339.515L201.278 231.41L216.435 363.876L182.717 234.88L148.999 363.876L164.157 231.41L86.1169 339.515L148.103 221.47L36.2811 294.083L136.724 206.402L6.22221 233.717L131.556 188.241L0 166.569L133.299 169.439L18.4548 101.707L141.715 152.537L59.0941 47.8919L155.669 139.816L116.429 12.3913L173.276 132.995L182.717 0Z" fill="#D9D9D9" fillOpacity="0.35"/>
    </svg>
  )
}
      ...
      <section className="mx-auto grid min-h-screen w-full max-w-[1600px] grid-cols-1 border-x border-black/10 lg:grid-cols-12">
        <header className="flex items-center justify-between gap-4 border-b border-black/10 px-6 py-5 lg:col-span-12 lg:px-12 lg:py-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <span className="hidden text-[14px] leading-[0.7] tracking-[0.25em] lg:block">●<br />●<br />●</span>
          </div>
        </header>
        ...
        <div className="order-1 border-t border-black/10 px-6 py-8 md:px-10 lg:order-2 lg:col-span-6 lg:border-r lg:px-12 lg:py-14" id="about">
          <motion.h1
          ...
          <div className="mt-12 grid grid-cols-1 gap-8 md:grid-cols-2" id="feature">
            {stats.map((stat) => (
